...
class ModuleCreator
{
    ...
    public function validateModule($input)
    {
        ...
        $newFields = $this->validateNewFields($input['fields']['new'] ? : array());
    ...
    private function validateNewFields(array $fields = array())
    {
        ...
        foreach ($fields as $field) {
            ...
            $validator = \Validator::make($field, $tmpField->getRules());
            if ($validator->fails()) {
    ...
    public function applyModuleChanges(array $input = array(), array $changeGroups = array())
    {
        ...
        foreach ($changeGroups as $groupType => $changesets) {
            if ($groupType === 'updated_fields') continue;
            foreach ($changesets as $changeset) {
                if (!is_array($changeset['changes']) || empty($changeset['changes'])) continue;

                $columnType = ArrayLib::getKey(ArrayLib::findByKeyValue($changeset['changes'], 'name', 'column_type'), 'new');
                $columnName = ArrayLib::getKey(ArrayLib::findByKeyValue($changeset['changes'], 'name', 'column_name'), 'new');
        ...
        foreach ($generatorMap as $key => $fields) {
            if (array_key_exists($key, $generatorMethods) && !empty($fields)) {
                MigrationGenerator::$generatorMethods[$key]($tableName, $fields);
            }
        }
        // Update model
        if (!empty($moduleChangeset['changes'])) {
            ...
            if ($moduleChangeset['item_id']) {
                ...
                file_put_contents($pathToModel, str_replace('extends Eloquent', 'extends Node', file_get_contents($pathToModel)));
